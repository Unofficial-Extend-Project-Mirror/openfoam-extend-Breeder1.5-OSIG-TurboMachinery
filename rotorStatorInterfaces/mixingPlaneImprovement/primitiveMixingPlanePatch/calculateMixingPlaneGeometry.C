...
List<scalar>& primitiveMixingPlanePatch::findMaxMinRadius()
{
    List<scalar> MinMaxRadius;


    return 
}

angleStruct primitiveMixingPlanePatch::findCircumferentialDistanceAverage(scalar maxRadius, scalar minRadius)
{
    // First search the pointField for the points lying on the min and maximum 
    // radius
    pointField minRadiusPoints(0);
    pointField maxRadiusPoints(0);
    angleStruct returnValue;
    returnValue.minAngleAtMinRadius =0;
    returnValue.minAngleAtMaxRadius =0;  
    returnValue.maxAngleAtMinRadius =0;
    returnValue.maxAngleAtMaxRadius =0;
    returnValue.deltaAngleAverage =0;

    forAll(originalPointsCCS,i)
    {
	if (originalPointsCCS[i].x() -minRadius < eps)
        {
	    minRadiusPoints.setSize(minRadiusPoints.size()+1);
            minRadiusPoints[minRadiusPoints.size()-1] = originalPointsCCS[i];
        }
 
	if (originalPointsCCS[i].x() -maxRadius < eps)
        {
	    maxRadiusPoints.setSize(maxRadiusPoints.size()+1);
            maxRadiusPoints[maxRadiusPoints.size()-1] = originalPointsCCS[i];
        }
    }
    // Now the max and min Radius points must be sorted for the azimuthal angle
    // Y is the azimuthal component
    minRadiusPoints = mergeSortComponent(minRadiusPoints, Vector::component::Y);
    returnValue.minAngleAtMinRadius =minRadiusPoints[0].y();
    returnValue.maxAngleAtMinRadius =minRadiusPoints[minRadiusPoints.size()-1);

    maxRadiusPoints = mergeSortComponent(maxRadiusPoints, Vector::component::Y);
    returnValue.minAngleAtMaxRadius =maxRadiusPoints[0].y();
    returnValue.maxAngleAtMaxRadius =maxRadiusPoints[minRadiusPoints.size()-1);

    // now the circumferential distance Average can be determined
    scalar circumferentialAverage = 0 ;
    scalar numberOfAverageParts = 0;
    forAll(maxRadiusPoints,i)
    {
        circumferentialAverage = maxRadiusPoints[i].y() + circumferentialAverage;
        numberOfAverageParts = numberOfAverageParts + 1;
    }
    forAll(minRadiusPoints,i)
    {
        circumferentialAverage = minRadiusPoints[i].y() + circumferentialAverage;
        numberOfAverageParts = numberOfAverageParts + 1;
    }
    returnValue.deltaAngleAverage = circumferentialAverage/ numberOfAverageParts;
    return (returnValue);
}
