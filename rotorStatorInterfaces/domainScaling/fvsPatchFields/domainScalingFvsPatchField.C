...
#include "domainScalingFvsPatchField.H"
...
template<class Type>
domainScalingFvsPatchField<Type>::domainScalingFvsPatchField
(
    const fvPatch& p,
    ...
    domainScalingPatch_(refCast<const domainScalingFvPatch>(p))
{
    if (!isType<domainScalingFvPatch>(p))
    {
        ...
        (
            ...
            ")\n",
            dict
        )   << "patch " << this->patch().index() << " not domainScaling type. "
            << "Patch type = " << p.type()
            << exit(FatalIOError);
    }
}
...
domainScalingFvsPatchField<Type>::domainScalingFvsPatchField
(
    const domainScalingFvsPatchField<Type>& ptf,
    const fvPatch& p,
    const DimensionedField<Type, surfaceMesh>& iF,
