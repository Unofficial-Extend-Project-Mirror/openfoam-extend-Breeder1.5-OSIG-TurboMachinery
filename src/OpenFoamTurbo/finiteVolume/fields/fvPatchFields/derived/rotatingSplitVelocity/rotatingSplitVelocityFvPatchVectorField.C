...
void rotatingSplitVelocityFvPatchVectorField::updateCoeffs()
{
    ...
    vector axisHat = omegaOne_/mag(omegaOne_);
    const vectorField& C = patch().Cf();

    vectorField rotationVelocity = omegaOne_ ^ ( C - axisHat*( axisHat & C ));
    ...
    forAll(C, facei)
     {
       ...
       if ( ( C[facei].z() > minZ_ ) and ( C[facei].z() < maxZ_ ) and ( r > minR_ ) and ( r < maxR_ ) )
          rotationVelocity[facei] = omegaTwo_ ^ ( C[facei] - axisHat*( axisHat & C[facei] ));
     }
